"""
    Complete Multimodal Emotion Recognition: Preprocessed EEG + Raw BVP Fusion
    ===========================================================================
    
    This script combines VERIFIED EEG training code (from Test_PP_EEG_Only.py)
    with RAW BVP loading and multimodal fusion.
    
    Pipeline:
    1. Load preprocessed EEG data (with baseline reduction) - VERIFIED ✅
    2. Load raw BVP data (original signals)
    3. Train EEG BiLSTM model - EXACT SAME CODE ✅
    4. Train BVP EMCNN model
    5. Train fusion model with cross-modal attention
    
    Datasets:
    - EEG: /kaggle/input/datasets/nethmitb/emognition-processed/Output_KNN_ASR (Preprocessed)
    - BVP: /kaggle/input/datasets/ruchiabey/emognition (Raw)
...
class Config:
    ...
    EEG_CHECKPOINT = "best_eeg_fusion.pt"
    ...
    BVP_LR = 1e-3
    BVP_PATIENCE = 10
    BVP_CHECKPOINT = "best_bvp_fusion.pt"
    ...
    FUSION_LR = 1e-3
    FUSION_PATIENCE = 10
    FUSION_CHECKPOINT = "best_fusion_complete.pt"
...
def load_bvp_data(data_root):
    """Load BVP data from RAW Empatica files."""
    print("\n" + "="*80)
    print("LOADING BVP DATA (RAW EMPATICA)")
    print("="*80)
...
class BVPEncoder(nn.Module):
    ...
    def __init__(self, feat_dim=5):
        ...
        self.out_dim = 64 * 3 + 16  # Will be set dynamically
...
def create_clip_based_split(eeg_clip_names, eeg_y, eeg_subjects, bvp_clip_names, bvp_y, bvp_subjects):
    """Create clip-based splits ensuring EEG and BVP alignment."""
    print("\n" + "="*80)
    print("CREATING HYBRID CLIP-BASED DATA SPLIT")
    print("="*80)
    
    # Find common clips between EEG and BVP
...
def get_clip_indices(clip_names_array, target_clips):
    """Get indices of windows belonging to target clips."""
...
def main():
    """Complete hybrid fusion pipeline: Preprocessed EEG + Raw BVP."""
    print("=" * 80)
    print("HYBRID MULTIMODAL EMOTION RECOGNITION PIPELINE")
    print("=" * 80)
    print(f"EEG Dataset (Preprocessed): {config.EEG_DATA_ROOT}")
    print(f"BVP Dataset (Raw): {config.BVP_DATA_ROOT}")
    ...
    # Step 1: Load EEG data (preprocessed)
    eeg_X_raw, eeg_y, eeg_subjects, eeg_file_ids, eeg_label_map, eeg_clip_names = load_eeg_data(config.EEG_DATA_ROOT)
    eeg_X_features = extract_eeg_features(eeg_X_raw)
    
    # Step 2: Load BVP data (raw)
    bvp_X_raw, bvp_y, bvp_subjects, bvp_file_ids, bvp_label_map, bvp_clip_names = load_bvp_data(config.BVP_DATA_ROOT)
    
    # Step 3: Create clip-based splits (ensures EEG-BVP alignment)
    ...
    print(f"\n📊 Final Split Sizes:")
    print(f"   EEG - Train: {len(eeg_split_indices['train'])}, Val: {len(eeg_split_indices['val'])}, Test: {len(eeg_split_indices['test'])}")
    print(f"   BVP - Train: {len(bvp_split_indices['train'])}, Val: {len(bvp_split_indices['val'])}, Test: {len(bvp_split_indices['test'])}")
    
    # Step 4: Train EEG model (preprocessed data)
    eeg_model, eeg_mu, eeg_sd = train_eeg_model(eeg_X_features, eeg_y, eeg_split_indices, eeg_label_map)
    
    # Step 5: Train BVP model (raw data)
    bvp_model = train_bvp_model(bvp_X_raw, bvp_y, bvp_split_indices, bvp_label_map)
    
    # Step 6: Prepare encoders for fusion
    print("\n" + "="*80)
    ...
    eeg_encoder.classifier = nn.Identity()  # Remove classifier head
    for p in eeg_encoder.parameters():
        p.requires_grad = False
    eeg_encoder.eval()
    print("✅ EEG encoder prepared (frozen)")
    
    # Create BVP encoder (remove classifier, freeze weights)
    bvp_encoder = BVPEncoder(feat_dim=5).to(config.DEVICE)
    bvp_state = torch.load(config.BVP_CHECKPOINT, map_location=config.DEVICE)
    
    # Load compatible weights from trained BVP model
    bvp_encoder_state = {}
    for k, v in bvp_state.items():
        if k.startswith('b1.') or k.startswith('b2.') or k.startswith('b3.') or k.startswith('feat_fc.'):
            bvp_encoder_state[k] = v
    bvp_encoder.load_state_dict(bvp_encoder_state, strict=False)
    
    # Infer BVP output dimension dynamically
    with torch.no_grad():
    ...
    for p in bvp_encoder.parameters():
        p.requires_grad = False
    bvp_encoder.eval()
    print(f"✅ BVP encoder prepared (frozen, out_dim={bvp_encoder.out_dim})")
    ...
    print("\n" + "=" * 80)
    print("🎉 COMPLETE HYBRID FUSION PIPELINE FINISHED! 🎉")
    print("=" * 80)
    print(f"✅ EEG model: {config.EEG_CHECKPOINT}")
    print(f"✅ BVP model: {config.BVP_CHECKPOINT}")
    print(f"✅ Fusion model: {config.FUSION_CHECKPOINT}")
    print("=" * 80)
    print("\n📊 Pipeline Summary:")
    print(f"   • EEG: Preprocessed KNN_ASR cleaned data with baseline reduction")
    print(f"   • BVP: Raw Empatica signals with standard preprocessing")
    print(f"   • Fusion: Cross-modal attention + gated fusion")
    print(f"   • Split: {len(splits['train_clips'])} train / {len(splits['val_clips'])} val / {len(splits['test_clips'])} test clips")
    print("=" * 80)
