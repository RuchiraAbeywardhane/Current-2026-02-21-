...
class Config:
    ...
    # Common parameters
    NUM_CLASSES = 3  # Changed from 4 to 3 (removed BASELINE and SURPRISE)
    SEED = 42
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Classification Mode
    USE_DUAL_BINARY = False  # True: Two binary classifiers (Arousal + Valence), False: Single 3-class classifier
    ...
    # Label mappings (3-class emotion system - BASELINE/NEUTRAL/SURPRISE removed)
    ...
    SUPERCLASS_ID = {"Q1": 0, "Q2": 1, "Q4": 2}  # 3 classes: Q1, Q2, Q4 (Q3 removed)
    IDX_TO_LABEL = ["Q1_Positive_Active", "Q2_Negative_Active", "Q4_Positive_Calm"]  # 3 labels
    
    # Dual Binary Classification Mappings (updated for 3 classes)
    # Arousal: High (Q1, Q2) vs Low (Q4)
    # Valence: Positive (Q1, Q4) vs Negative (Q2)
    AROUSAL_MAP = {0: 1, 1: 1, 2: 0}  # Q1,Q2->High(1), Q4->Low(0)
    VALENCE_MAP = {0: 1, 1: 0, 2: 1}  # Q1,Q4->Positive(1), Q2->Negative(0)
...
def load_eeg_data(data_root):
    ...
    patterns = [os.path.join(data_root, "*_STIMULUS_MUSE.json"),
...
def main():
    ...
    # Convert clip-based splits to indices
    eeg_split_indices = {
    ...
    # Step 5: Train BVP model
    bvp_model = train_bvp_model(bvp_X_raw, bvp_y, bvp_split_indices, bvp_label_map)
    
    # Step 6: Prepare fusion encoders
    print("\n" + "="*80)
    print("PREPARING FUSION ENCODERS")
    print("="*80)
    ...
    print(f"✅ Split indices: {config.SPLIT_FILE}")
    print("=" * 80)
