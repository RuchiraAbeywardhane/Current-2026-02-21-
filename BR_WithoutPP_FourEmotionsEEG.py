...
def main():
    ...
    print("\n" + "="*80)
    print("CREATING STRATIFIED SUBJECT-INDEPENDENT SPLIT (IDENTICAL TO MULTIMODAL)")
    print("="*80)
    
    subject_to_clips = defaultdict(list)
    clip_to_label = {}
    
    # Build clip mapping exactly like multimodal pipeline
    for clip, label in zip(eeg_clip_names, eeg_y):
        subject = clip.split('_')[0]
        subject_to_clips[subject].append(clip)
        if clip not in clip_to_label:
            clip_to_label[clip] = label
    
    subjects = sorted(subject_to_clips.keys())
    
    # Majority label per subject (same logic)
    subject_to_label = {}
    
    for subject in subjects:
        subject_clips = subject_to_clips[subject]
        labels = [clip_to_label[c] for c in subject_clips]
