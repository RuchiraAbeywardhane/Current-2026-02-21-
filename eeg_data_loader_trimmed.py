...
TRIM_START_SEC = 15.0  # Remove first 3 seconds (reduced from 10)
TRIM_END_SEC = 15.0    # Remove last 3 seconds (reduced from 10)
...
def trim_temporal_edges(signal, fs, trim_start_sec=10.0, trim_end_sec=10.0):
    ...
    if total_length <= (trim_start_samples + trim_end_samples):
        print(f"      ⚠️  Signal too short ({total_length} samples) to trim {trim_start_samples + trim_end_samples} samples")
...
def load_baseline_files(files, data_root, config):
    ...
    print(f"   ✅ Loaded {len(baseline_dict)} baseline recordings (with trimming)")
...
def load_eeg_data(data_root, config):
    ...
    **KEY MODIFICATION**: Removes first 3 seconds and last 3 seconds from each
    ...
    print("="*80)
    print(f"⏱️  Trimming: First {TRIM_START_SEC}s and Last {TRIM_END_SEC}s removed")
    print("="*80)
    ...
    print(f"   ✅ Successfully trimmed: {trimmed_count} files")
